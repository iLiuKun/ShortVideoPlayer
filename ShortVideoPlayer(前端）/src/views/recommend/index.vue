  ...
  <div class="container">
    ...
    <div class="content">
      <div class="funny-videos">
        <div v-for="video in recommedVideos" :key="video.id" @click="playVideo(video.videoUrl)" class="video-thumbnail">
...
onMounted(async () => {
  try {
    const response = await sysVideoApi.getRecommedVideoList();
    recommedVideos.value = response.data.data; 
  } catch (error) {
  ...
  const handleKeyDown = (event: KeyboardEvent) => {
    ...
    if (event.key === 'ArrowUp' || event.key === 'w') {
      // 按下了上箭头键
      currentVideoIndex.value = (currentVideoIndex.value - 1 + recommedVideos.value.length) % recommedVideos.value.length;
      currentVideoUrl.value = recommedVideos.value[currentVideoIndex.value].videoUrl;
    } else if (event.key === 'ArrowDown' || event.key === 's') {
      // 按下了下箭头键
      currentVideoIndex.value = (currentVideoIndex.value + 1) % recommedVideos.value.length;
      currentVideoUrl.value = recommedVideos.value[currentVideoIndex.value].videoUrl;
    }
  };
...
const playVideo = (videoUrl) => {
  ...
  const index = recommedVideos.value.findIndex((video) => video.videoUrl === videoUrl);
  if (index !== -1) {
